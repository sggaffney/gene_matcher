...
import os
import pandas as pd
from initialize_db import build_ref_db, build_user_db, import_user_list
...
class Matcher(object):
    ...
    def run(self):
        self._run_matches()
        (self.n_matched, self.n_failed) = self._export_matches()
    ...
    def _cal_fix(hugo, entrez, user_symbol, chromosome):
        return """update user set hugo = {hugo!r}, entrez_id = {entrez}
            ...
            and chromosome = {chrom!r};""".format(hugo=hugo, entrez=entrez,
                                                user_sym=user_symbol,
                                                chrom=chromosome)
    ...
    def _export_matches(self):
        ...
        file_root = os.path.basename(self.file_path)
        full_path = file_root + '_full.txt'
        good_path = file_root + '_converted.txt'
        fail_path = file_root + '_failed.txt'

        # all rows
        df = pd.read_sql("select user_symbol, chromosome, case when hugo is null "
        ...
        # successful rows
        df = pd.read_sql("select * from user where hugo is not null "
                         "order by id;",
                         self.engine, coerce_float=False)
        df[df.columns.drop('id')].to_csv(good_path, sep='\t', index=False)
        n_matched = len(df)

        # failed rows
        df = pd.read_sql("select * from user where hugo is null;", self.engine,
                         coerce_float=False)
        if len(df):
            df[['user_symbol', 'chromosome']].to_csv(fail_path, sep='\t',
                                                     index=False, header=False)
        n_failed = len(df)
        return n_matched, n_failed

def run_matcher(user_path):
    build_ref_db(ref_file=ref_file)
    engine, meta, refs, user = build_user_db(ref_file=ref_file, echo=False)
    file_path = import_user_list(user_path=user_path, engine=engine)

    matcher = Matcher(engine, file_path)
    matcher.run()  # match and save files.
